            ...
            <div class="form-value">
                ...
                if (isset($_POST["login"])) {
                    ...
                    if (mysqli_num_rows($emailResult) > 0) {
                        ...
                        $checkVerificationSql = "SELECT * FROM users WHERE email = '$email' AND verified = 1";
                        $verificationResult = mysqli_query($conn, $checkVerificationSql);
                        ...
                        if (mysqli_num_rows($verificationResult) > 0) {
                            ...
                            if ($user) {
                                ...
                                echo "<div class='alert alert-danger alert-dismissible fade show' role='alert'>Error retrieving user information.<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button></div>";
                            }
                        } else {
                            // Email is not verified, send a new OTP
                            $newOtp = generateOtp();
                            $newOtpExpiresAt = date('Y-m-d H:i:s', strtotime('+10 minutes'));

                            // Update database with new OTP and expiration time
                            $updateOtpSql = "UPDATE users SET otp = '$newOtp', otp_expires_at = '$newOtpExpiresAt' WHERE email = '$email'";
                            mysqli_query($conn, $updateOtpSql);

                            // Send new verification email with OTP
                            $to = $email;
                            $subject = "New OTP for Email Verification";
                            $message = "Your new OTP which will expire after 10 min for email verification is: $newOtp";
